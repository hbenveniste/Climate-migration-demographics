...
replace modelnb = 10 if model == "T,S*(age+edu+sex)"
replace modelnb = 11 if model == "T,S placebo*(age+edu)"
...
twoway rbar rsqplot_lqt rsqplot_lqtx modelnb, barw(.5) fcolor(gs5) lcolor(black) ///
	...
	xlabel(1 "T" 2 "S" 3 "T,S" 4 "T,S * climate zone" 5 "T,S * age" 6 "T,S * edu" 7 "T,S * sex" 8 "T,S * (age+edu)" 9 "T,S * climzone * (age+edu)" 10 "T,S * (age+edu+sex)" 11 "T,S placebo * (age+edu)", angle(50) labsize(small)) ///
	xtitle("") ytitle("Out-of-sample performance (R2)") ylabel(-0.001(0.001)0.002,labsize(small)) ///
...
replace modelaltnb = 2 if model == "T,S*(age+edu)"
replace modelaltnb = 3 if model == "T1,S3*(age+edu)"
replace modelaltnb = 4 if model == "T,S placebo*(age+edu)"
label define modelname 1 "T,S cubic" 2 "T,S cubic*(age+edu)" 3 "T linear, S cubic*(age+edu)" 4 "T,S cubic placebo*(age+edu)", modify
