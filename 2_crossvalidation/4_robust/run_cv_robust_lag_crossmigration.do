...
* Model performing best out-of-sample: T,S contemporaneous and lagged by 1 year, cubic, per climate zone, age and education
...
quietly {
	gen model = "(T3,S3+l1)*(climzone+age+edu)"
	if "$metric" == "rsquare" {
...
replace modelnb = 4 if model == "(T3,S3+l1)*(climzone+age+edu)"
label define modelname 1 "T,S" 2 "T,S * (clim+age+edu)" 3 "T,S + lag1" 4 "(T,S+lag1)* (clim+age+edu)" , modify
...
graph export "$res_dir/2_Crossvalidation_crossmig/FigS12a_cv_crosslag.png", ///
