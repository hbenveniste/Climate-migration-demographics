...
local xmax245 = ceil(r(max) / 10) * 10
local xmin245 = floor(r(min) / 10) * 10
...
local xmax370 = ceil(r(max) / 10) * 10
local xmin370 = floor(r(min) / 10) * 10
local xmeanh370 = r(mean)
local xstep370 = (0 - `xmin370') / 5
...
forv j=4(-1)1 {
	forv i=1/4 {
		...
		if `i' == 1 & `j' == 1 {
			local ylab "ytitle(`ytit', size(large)) ylabel(, nolabel notick)"
			local xlab "xtitle(`xtit', size(large)) xlabel(, labsize(med))"
		}
		if `i' > 1 & `j' == 1 {
